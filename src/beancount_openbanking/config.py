...
class AccountConfig(BaseModel):
    """Configuration for a single imported bank account."""
...
def load_config(
    ...
    env = _load_env_values(config_path, config_dict, env_files=env_files)
    expanded_config = _expand_config_values(config_dict, env)
    expanded_config.pop("env", None)
    expanded_config.pop("env_files", None)
    if "provider" not in expanded_config and default_provider is not None:
        expanded_config["provider"] = default_provider
    config = CONFIG_ADAPTER.validate_python(expanded_config)
